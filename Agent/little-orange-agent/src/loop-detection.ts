...
const CRITICAL_THRESHOLD = 8  //严重阈值，超过就停止工具的调用
const BREAKER_THRESHOLD = 20  //熔断阈值，超过就直接停止循环

// 指纹计算
function stableStringify(value: unknown): string {
  if (value === null || typeof value !== 'object') {// 非对象类型，直接返回字符串
    return JSON.stringify(value)
  }
  if (Array.isArray(value)) {// 数组类型，递归调用
    return `[${value.map(stableStringify).join(',')}]` // ['xxx,yyy']
  }

  const keys = Object.keys(value as Record<string, unknown>).sort();// 对象的键排序
  return `{${keys.map(k => `${JSON.stringify(k)}:${stableStringify((value as any)[k])}`).join(',')}}`;  // 对象类型，递归调用
}

function hash(input: string): string {
  return createHash('sha256').update(input).digest('hex').slice(0, 16);// 取前16位，作为指纹
}

export function hashToolCall(toolName: string, params: unknown): string {// 计算工具调用的指纹
  return `${toolName}:${hash(stableStringify(params))}`;
}

export function hashResult(result: unknown): string {// 计算工具调用结果的指纹
...
export function recordResult(toolName: string, params: unknown, result: unknown): void {// 记录工具调用结果
