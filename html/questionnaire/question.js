...
let answerList = [];//存放用户选择的答案
const list = document.querySelector('.list')  // ul
let selectId = null // 记录当前选中的答案id
...
goResult.addEventListener('click', () => {
    ...
    //对比输入的答案answerList和正确的选项topic_answer_id
    let score = 0
    for (let i = 0; i < questionList.length; i++) {
        if (questionList[i].topic_answer_id == answerList[i]) {
            score++
        }
    }
    //2.弹出得分
...
function showQuestion(arr, current) {
    ...
    for (let i = 0; i < topic_answer.length; i++) {
        const li = `
      <li onclick="selectTtem(${topic_answer[i].topic_answer_id})">
